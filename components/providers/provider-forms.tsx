...
export function CreateProviderForm({ action }: { action: Action }) {
  ...
  return (
    <form className="record-form" action={formAction}>
      <div><label htmlFor="kind">Provider type</label><select id="kind" name="kind"><option value="company">Registered company</option><option value="individual_owner">Individual vehicle owner</option></select></div><div><label htmlFor="declaredVehicles">Vehicles owned</label><input id="declaredVehicles" name="declaredVehicles" type="number" min="1" defaultValue="1" required/></div><div className="field-span"><label htmlFor="name">Company or owner name</label><input id="name" name="name" required minLength={2} /></div>
