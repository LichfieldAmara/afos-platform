...
const providerSchema=z.object({name:z.string().trim().min(2),kind:z.enum(["company","individual_owner"]),registrationNumber:z.string().trim().max(80),contactName:z.string().trim().min(2),contactPhone:z.string().trim().min(5),contactEmail:z.string().trim().email().or(z.literal("")),declaredTrucks:z.coerce.number().int().min(0).max(1000),declaredTrailers:z.coerce.number().int().min(0).max(1000),areas:z.string().trim().max(500)}).refine(x=>x.declaredTrucks+x.declaredTrailers>0,{message:"Enter at least one truck or trailer."}).refine(x=>x.kind!=="company"||x.registrationNumber.length>=2,{message:"Enter the company registration number."});
...
export async function createProvider(
  ...
  const parsed = providerSchema.safeParse({
    ...
    kind:formData.get("kind"),contactEmail:formData.get("contactEmail")??"",declaredTrucks:formData.get("declaredTrucks"),declaredTrailers:formData.get("declaredTrailers"),areas:formData.get("areas")??"",
  });
  if (!parsed.success) return { message: parsed.error.issues[0]?.message ?? "Check the provider details." };
  const supabase = await createServerSupabaseClient();
  const { error } = await supabase.rpc("create_transport_provider_v3", {
    provider_name: parsed.data.name,
    registration_number: parsed.data.registrationNumber,
    contact_name: parsed.data.contactName,
    contact_phone: parsed.data.contactPhone,
    provider_kind:parsed.data.kind,contact_email:parsed.data.contactEmail,declared_trucks:parsed.data.declaredTrucks,declared_trailers:parsed.data.declaredTrailers,operating_areas:parsed.data.areas,
  });
...
export async function registerDriver(_s:ProviderActionState,f:FormData):Promise<ProviderActionState>{await requirePlatformRole(["afos_operations","afos_administrator"]);const p=z.object({providerId:z.uuid(),name:z.string().trim().min(2),phone:z.string().trim().min(5),license:z.string().trim().min(2),expiry:z.string()}).safeParse({providerId:f.get("providerId"),name:f.get("name"),phone:f.get("phone"),license:f.get("license"),expiry:f.get("expiry")??""});if(!p.success)return{message:"Enter the driver's name, phone and licence."};const db=await createServerSupabaseClient();const{error}=await db.rpc("register_provider_driver",{target_provider_id:p.data.providerId,driver_name:p.data.name,driver_phone:p.data.phone,license:p.data.license,license_expiry:p.data.expiry||null});if(error)return{message:error.message.includes("unique")?"That licence is already registered.":"Driver could not be registered."};revalidatePath("/app/providers");return{message:"Provider driver registered.",success:true};}
